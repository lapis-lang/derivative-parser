...
export class Lam extends Term {
  constructor(readonly param: string, readonly body: Term) {
    super();
  }
  override print(): string {
...
export class Let extends Term {
  constructor(readonly name: string, readonly def: Term, readonly body: Term) {
    super();
  }
  override print(): string {
...
export class LambdaGrammar extends Grammar<{ expr: Term }> {
  ...
  @rule
  protected get letExpr(): Parser<Term> {
    return this.seq(
      this.kw("let"),
      this.ws1,
      this.ident,
  ...
  @rule
  protected get lambda(): Parser<Term> {
    return this.seq(
      this.lambdaHead,
      this.ident,
      this.ws,
  ...
  @rule
  protected get app(): Parser<Term> {
    return this.or(
      this.seq(this.app, this.ws1, this.atom).map(([fn, , arg]) =>
        new App(fn, arg)
      ),
      this.atom,
    );
  ...
  @rule
  protected get atom(): Parser<Term> {
    return this.or(
